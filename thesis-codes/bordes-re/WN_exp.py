...
#             marge=state.marge, rel=False)
   ...
   # ranklfunc = RankLeftFnIdx(simfn, embeddings, leftop, rightop,
    ...
    #        subtensorspec=state.Nsyn)
    rankrelfunc = RankRelFnIdx(simfn, embeddings, leftop, rightop, offset=state.Nreloffset ,subtensorspec=state.Nent)
	...
	order = np.random.permutation(trainl.shape[1])
        ...
        for i in range(state.nbatches):
            ...
            # training iteration
            outtmp = trainfunc(state.lremb, state.lrparam / float(batchsize), tmpl, tmpr, tmpo, tmpnl, tmpnr)
...
#                     validlidx, validridx, validoidx)
            resvalid = RankingScoreRelIdx(rankrelfunc, validlidx, validridx, validoidx,offset=state.Nreloffset)
            state.valid = np.mean(resvalid[0] + resvalid[1])
#             restrain = RankingScoreIdx(ranklfunc, rankrfunc,
#                     trainlidx, trainridx, trainoidx)
			
            restrain = RankingScoreRelIdx(rankrelfunc, trainlidx, trainridx, trainoidx,offset=state.Nreloffset)
...
#                         testlidx, testridx, testoidx)
                restest = RankingScoreRelIdx (rankrelfunc, testlidx, testridx, testoidx,offset=state.Nreloffset)
...
def launch(datapath='', dataset='', Nent=40961,
        Nsyn=40943, Nrel=18,Nreloffset=0, loadmodel=False, loademb=False, op='Unstructured',
