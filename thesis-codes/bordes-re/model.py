...
def RankRelFnIdx(fnsim, embeddings, leftop, rightop, offset=0,subtensorspec=None):
    embedding, relationl, relationr = parse_embeddings(embeddings)
    """
    This function returns a Theano function to measure the similarity score of
    ...
    if subtensorspec is not None:
        # We compute the score only for a subset of entities
        rell = (relationl.E[:, offset:subtensorspec]).T
        relr = (relationr.E[:, offset:subtensorspec]).T
    else:
        rell = embedding.E.T
        relr = embedding.E.T
    # hack to prevent a broadcast problem with the Bilinear layer
...
def RankingScoreRelIdx(so, idxl, idxr, idxo, offset=0):
    """
    ...
    erro = []
    for l, o, r in zip(idxl, idxo, idxr):
    	#print np.argsort((so(l, r)[0]).flatten())[::-1]
    	if (o<offset):
    		continue
        erro += [np.argsort(np.argsort((so(l, r)[0]).flatten())[::-1]).flatten()[o-offset] + 1]
